...
| `charter handoff <plan.charter.md> -o <out.md> [--answers <answers.json>] [--fail-if-needs-human] [--manifest]` | Convert the plan's `:::` directives to **plain CommonMark** for the **autonomous** Guardrails `plan-breakdown` path. (That path is also called "headless" — an unrelated sense of the word from the `charter headless` verb above. `handoff` writes no forensic record; `headless` writes no CommonMark. **This is the verb that feeds Guardrails**; reaching for `charter headless` produces no `plan.md` and exits 0, so the run reports success while Guardrails gets nothing.) `--fail-if-needs-human` still writes the output and exits **2** when a decision nobody made survives `--answers`. `--manifest` also writes `<out-stem>.manifest.json` — a **chain-of-custody** record of which inputs produced this exact output, from the same resolution pass. Neither flag implies the other. |
| `charter verify <handoff.md>` | **Read-only** custody check on a pair `handoff --manifest` wrote: manifest `planSha256` vs the in-band plan stamp, `answersSha256` vs the answers stamp, `handoffSha256` vs the file as it now stands, and the manifest's `questions[]` vs the ids and Answered/Open states **the document actually carries** (without that last one, a manifest saying `"answered": true` beside an *Open question* passes every hash join). Pass the **handoff**, not the manifest and not the `.charter.md`. `0` every join holds and nothing is outstanding · **`2` a join disagreed OR the manifest records `gate.needsHuman`** · `1` it could not answer — not a verdict. A CRLF rewrite and an added trailing newline are **named**, never excused. **What it cannot do:** both files are writable by the same party, so it detects **inconsistency, never incorrectness** — it does not check answer *values*, does not prove the handoff reached Guardrails unmodified, and does not prove a human reviewed anything. Never quote a green `verify` as evidence a run was proper. (The review-side checks are `charter review verify`, a different verb, not built.) |
...
| `charter skills install [--project] [--force] [--overwrite-tracked] [--target <dir>]` | Install the bundled `charter`, `charter-format` + `charter-drain` skills so Guardrails `plan-breakdown` can discover them. `--target` names an explicit directory; `--project` uses `./.claude/skills`. `--force` overwrites an existing copy, and is **refused** when the target holds git-tracked skill source with uncommitted changes — `--overwrite-tracked` is the human's explicit override for that, so **never reach for it on your own**: the work it destroys is the part no `git checkout` brings back. |
...
**If the run has to be auditable afterwards, add `--manifest`:**

```
charter handoff plan.charter.md -o plan.md --answers answers.json --fail-if-needs-human --manifest
```

It writes `plan.manifest.json` beside `plan.md` — the sha256 of the plan, the answers and the handoff, every
question's resolved answer with whether it came from the plan (`inline`) or the file (`answers-file`), and the
gate's verdict — all from the **same resolution pass** that wrote the CommonMark, so the two can never
disagree. It is opt-in and changes no exit code. The flattened plan also carries both hashes in-band, as two
trailing HTML comments, so a stale manifest is detectable from the two files alone. Contract:
