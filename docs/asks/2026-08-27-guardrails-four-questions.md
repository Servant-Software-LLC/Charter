...
|---|---|---|---|
| 1 | In-band `<!-- charter: plan-sha256=<hex> -->` | trailing comment in the flattened CommonMark | ❌ master only — **corrected, see below** |
...
> **Correction, 2026-08-27 (Charter side), after the answers came back.** Row 1 originally read
> "✅ v0.24.0". **That was wrong.** `git show v0.24.0:src/Charter.Core/HandoffMarkdown.cs` contains
> **neither** stamp constant and **no** delegated marker — zero matches for `sha256` or `Delegated`.
> The released 0.24.0 flatten carries **no in-band stamp at all**. Only row 3 shipped.
>
> **Why it does not change your answer, and where it helps:** both stamps are unreleased, so both are
> still free to change — which is strictly more room than you were told you had, and it matches what you
> asked for in Q4 note 1 ("neither shape needs freezing on our account"). It does sharpen the headline in
> your direction: **every surface #496 gates on is unreleased**, not three of four.
>
> The cause is the same class of error your Q1 correction names — a claim transcribed from a summary
> instead of re-derived from the tree. We verified against the tag this time.
