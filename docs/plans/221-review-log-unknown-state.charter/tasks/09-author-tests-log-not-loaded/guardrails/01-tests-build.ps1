...
if ($buildExit -ne 0) {
    ...
    Write-Output "tests/Charter.Browser.Tests/Charter.Browser.Tests.csproj does not compile - ReviewLogNotLoadedTests.cs has a syntax or reference error ($($csErrors.Count) CS error(s)). The tests must COMPILE and FAIL; not compiling is a mistake to fix, not a TDD red."
    exit 1
}
