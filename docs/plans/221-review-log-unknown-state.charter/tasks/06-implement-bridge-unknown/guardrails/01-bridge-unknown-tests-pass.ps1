# catches: a bridge that swallows the Unknown outcome before the panel can see it, or one that 'fixes' FindComment by reporting Unknown for a genuinely absent id. The --filter names this pair's OWN selector, never a plan-wide
...
if ($testExit -ne 0) {
    ...
    Write-Output "The bridge does not surface Unknown, or it broke one of the discriminators - a present read must still serve its comments and a genuinely absent id must still report not-found (see failure details above)."
    exit 1
}
...
$ran = ([regex]::Matches(($out | Out-String), '(?:Passed|Failed):\s*(\d+)') |
 ...
 if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified nothing. The --filter matched no tests or is malformed. Check it against the class BridgeUnknownTests."
    exit 1
}
