# catches: a read that still conflates an absent .review/ with an empty one, or an UNBOUNDED retry that hangs instead of settling. The --filter names this pair's OWN selector, never a plan-wide
...
if ($testExit -ne 0) {
    ...
    Write-Output "The three-state read is not implemented to spec - an absent directory must read Unknown, an existing empty one must still read Empty, and the retry must be BOUNDED (see failure details above)."
    exit 1
}
...
$ran = ([regex]::Matches(($out | Out-String), '(?:Passed|Failed):\s*(\d+)') |
 ...
 if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified nothing. The --filter matched no tests or is malformed. Check it against the class ReviewLogUnknownStateTests."
    exit 1
}
