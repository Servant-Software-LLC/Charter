...
$out = dotnet build tests/Charter.Cli.Tests/Charter.Cli.Tests.csproj -c Release --nologo -v q 2>&1
...
if ($buildExit -ne 0) {
    ...
    Write-Output "tests/Charter.Cli.Tests/Charter.Cli.Tests.csproj does not compile - ReviewLogDrainUnknownTests.cs has a syntax or reference error ($($csErrors.Count) CS error(s)). The tests must COMPILE and FAIL; not compiling is a mistake to fix, not a TDD red."
    exit 1
}
