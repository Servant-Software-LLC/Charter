...
# DECLARED EXEMPTIONS: none. hydrateLog applies any view unconditionally today, so every row is red on the current tree. If a row here goes green before the
...
$manifest = [ordered]@{
    ...
    'an Unknown view does not drop focus to body'                    = 'An_unknown_view_does_not_drop_focus_to_body'
    'the declined Unknown view is reported out loud'                 = 'The_declined_unknown_view_is_reported_out_loud'
    'a genuinely empty view still empties the panel'                 = 'A_genuinely_empty_view_still_empties_the_panel'
}
...
if ($recorded.Count -lt 1) {
    Write-Output "the TRX records ZERO executed tests - the --filter '$filter' matched nothing (is the Feature trait ReviewLogUnknownPanel on every test method? Category=BrowserAcceptance alone selects every browser test and cannot discriminate this pair), the filter is malformed, or every match is skipped. This is NOT a finding about the tests: do NOT rewrite them."
    exit 1
}
