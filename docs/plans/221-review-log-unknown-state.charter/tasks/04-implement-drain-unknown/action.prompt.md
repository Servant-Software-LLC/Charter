...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  key -- the name of the directory this task.json lives in (e.g. `04-implement-drain-unknown`), NOT the
  stableId. The harness REJECTS a fragment keyed by anything else (every attempt), so:
  `{ "04-implement-drain-unknown": { "someKey": "someValue" } }`.
...
Make the drain translate an Unknown read into **exit 4** so the tests authored by
`03-author-tests-drain-unknown` pass.

Edit `src/Charter.Server/ReviewLogDrain.cs` and `src/Charter.Cli/PollCommand.cs`.

**Scope boundary (harness-enforced):** Write only to `src/Charter.Server/ReviewLogDrain.cs` and `src/Charter.Cli/PollCommand.cs`. After this task completes, the harness
...
### The principle is already in the file you are editing

`ReviewLogDrain` reads the plan with `TryReadPlan`, and its own comment says: *"an unreadable plan
orphans everything and answers `unknown` for every base, which is the honest answer when nothing can be
verified."* It applies that to the **plan** and not to the **log directory**. Grep for that comment: you
are extending a rule this file already holds, not introducing one, and the two should read consistently
when you are done.

### What must not change

`0` (drained), `2` (a queue was found and it was empty), `3` (no session and no readable log) and `5`
(apply refused) keep their meanings exactly. This makes `4` reachable for a cause that previously
reported `2`; it redefines nothing. The exit-code contract is public and agents branch on it.
