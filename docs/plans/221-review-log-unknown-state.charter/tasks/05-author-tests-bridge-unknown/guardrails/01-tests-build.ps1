...
if ($buildExit -ne 0) {
    Write-Output ""
    Write-Output "tests/Charter.Server.Tests/Charter.Server.Tests.csproj does not compile - BridgeUnknownTests.cs has a syntax or reference error. The tests must COMPILE and FAIL; not compiling is a mistake to fix, not a TDD red."
    exit 1
}
