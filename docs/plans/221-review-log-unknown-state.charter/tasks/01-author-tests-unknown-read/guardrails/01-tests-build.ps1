...
$out = dotnet build tests/Charter.Server.Tests/Charter.Server.Tests.csproj -c Release --nologo -v q 2>&1
...
if ($buildExit -ne 0) {
    ...
    Write-Output "tests/Charter.Server.Tests/Charter.Server.Tests.csproj does not compile - ReviewLogUnknownStateTests.cs has a syntax or reference error ($($csErrors.Count) CS error(s)). The tests must COMPILE and FAIL; not compiling is a mistake to fix, not a TDD red."
    exit 1
}
