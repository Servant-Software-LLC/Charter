...
# DECLARED EXEMPTIONS: none. Every behaviour below drives an outcome the read cannot yet report, against NotImplementedException stubs. If a row here goes green before the
#          implementation lands, that test is not driving the subject: fix the test, never add an exemption.
$ErrorActionPreference = 'Continue'
$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$filter = 'Category=ReviewLogUnknownState&FullyQualifiedName~ReviewLogUnknownStateTests'

$manifest = [ordered]@{
    'a directory with logs reads as present'                         = 'A_directory_with_logs_reads_as_present'
    'an existing directory with no logs reads as EMPTY'              = 'An_existing_directory_with_no_logs_reads_as_empty'
    'a missing directory reads as UNKNOWN, not empty'                = 'A_missing_directory_reads_as_unknown_not_empty'
    'Unknown and Empty are distinguishable by a caller'              = 'Unknown_and_empty_are_distinguishable_by_a_caller'
    'a transient failure inside the bound still reads present'       = 'A_transient_failure_inside_the_retry_bound_still_reads_present'
    'a permanently missing directory settles as Unknown, bounded'    = 'A_permanently_missing_directory_settles_as_unknown_within_the_bound'
}
...
if ($recorded.Count -lt 1) {
    Write-Output "the TRX records ZERO executed tests - the --filter '$filter' matched nothing (is the Category trait ReviewLogUnknownState on the class, as every sibling file in that project does it?), the filter is malformed, or every match is skipped. This is NOT a finding about the tests: do NOT rewrite them."
    exit 1
}
