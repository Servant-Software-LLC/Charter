# BASELINE (#181), plan-level, AREA-SCOPED: is Charter.Cli.Tests already green before this plan touches it?
# catches: a pre-existing red that the terminal <plan>/guardrails/02-all-tests-pass.ps1 would otherwise
#          blame on this plan - AFTER every task has burned its spend. This area is modified by tasks 03/04.
...
$filter = 'Category!=ReviewLogDrainUnknown'

# NO -v q on a test command: it suppresses the Error Message/Expected/Actual/Stack Trace block (#179).
$out = dotnet test tests/Charter.Cli.Tests/Charter.Cli.Tests.csproj -c Release --nologo --filter $filter 2>&1
...
if ($testExit -ne 0) {
    ...
    Write-Output "Charter.Cli.Tests is ALREADY RED before this plan has changed anything. Fix that first: every red the terminal gate reports at the end of this run would otherwise be attributed to the plan."
    exit 1
}
...
if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed in Charter.Cli.Tests - the test host did not run, so no baseline was established. This is NOT a green baseline."
    exit 1
}

Write-Output "Baseline established: $ran tests executed and Charter.Cli.Tests is green before the plan runs."
exit 0
